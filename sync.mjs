...
const goodName = (s) => { const t = (s || "").trim(); return t && !t.includes(ITEM_PLACEHOLDER) ? t : null; };

// načíta už uložené názvy z našej DB (lacné Supabase volanie, nie SF), aby sme
// detail zo SF doťahovali len pre objednávky, ktoré ešte poriadny názov nemajú
async function storedOrderNames() {
  try {
    const res = await fetch(`${SB_URL}/rest/v1/order?select=sf_order_id,items_summary`, {
      headers: { apikey: SB_KEY, Authorization: `Bearer ${SB_KEY}` },
    });
    const d = await res.json(); const map = {};
    (Array.isArray(d) ? d : []).forEach((r) => { if (r.sf_order_id) map[String(r.sf_order_id)] = r.items_summary; });
    return map;
  } catch { return {}; }
}

async function orderItemsName(inv, stored) {
  const fromList = goodName(inv.items_name);
  if (fromList) return fromList;                 // zoznam vrátil dobrý názov
  const fromDb = goodName(stored);
  if (fromDb) return fromDb;                      // už máme uložený dobrý názov → nesiahaj na SF
  try {                                           // až teraz dotiahni detail (raz, kým nie je vyriešený)
    const j = await sf(`/invoices/view/${inv.id}.json`);
    const items = j.InvoiceItem || (j.Invoice && j.Invoice.InvoiceItem) || [];
    const names = items.map((it) => (it.name || "").trim()).filter((n) => goodName(n));
    if (names.length) return names.join(", ");
  } catch (e) { /* detail nedostupný — necháme čo máme */ }
  return (inv.items_name || "").trim() || stored || null;
}
...
(async () => {
  ...
  // ----- FAKTÚRY -----
  const STORED = await storedOrderNames();   // uložené názvy (lacné, raz) – aby sme zbytočne nesahali na SF
  for (const type of ["order", "proforma", "regular"]) {
    ...
    for (const r of recs) {
      ...
      if (type === "order") {
        const itemsName = await orderItemsName(inv, STORED[String(inv.id)]);
        orders.push({
          code: c,
